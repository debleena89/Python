...
def check_emptyness_of_Buchi_automata(intersection_matrix_for_buchi_automata):
    ...
    len_of_accepting_states = len(set_of_accepting_states)
    for accepting_state_index in range(0,len_of_accepting_states):
        source[0] = set_of_accepting_states[accepting_state_index]
        loop_limit = 1
...
def new_product():
    ...
    #print(intersection_matrix_for_buchi_automata)
    if  not set_of_accepting_states:
       print("Is the language of this NBA empty ?? : YES")
    else:
       start_time = time.time()        
